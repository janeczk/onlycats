<div>
    @if($posts->isEmpty())
        @if(request()->routeIs('profile.show') || request()->routeIs('profile.public'))
            <p class="text-center text-gray-500">This user did not post anything yet.</p>
        @endif
    @else
        <div class="space-y-6 w-full">
            @foreach($posts as $post)
                <a href="{{ route('posts.show', ['post' => $post->id, 'referer' => request()->route()->getName(), 'query' => request('query')]) }}" class="block w-full bg-white border-y border-gray-200 rounded-none p-6 hover:shadow-lg">
                    <!-- Header Section -->
                    <div class="flex justify-between items-center">
                        <!-- User Details -->
                        <div class="flex items-center space-x-3">
                            <!-- Avatar -->
                            <x-avatar :user="$post->user" size="50" class="rounded-full" />

                            <!-- Name and Username -->
                            <div>
                                <div class="text-gray-900 font-bold">
                                    {{ $post->user->name }} <!-- Username -->
                                </div>
                                <div class="text-gray-600">
                                    {{$post->user->username }} <!-- Display Name -->
                                </div>
                            </div>
                        </div>

                        <!-- Time -->
                        <div class="text-sm text-gray-500">
                            {{ $post->created_at->diffForHumans() }} <!-- Time ago -->
                        </div>
                    </div>


                    <div class="post">
                        <!-- Treść posta -->
                        <p class="text-gray-800 mb-4 pt-4">{{ $post->content }}</p>
                        <!-- Sprawdź, czy post zawiera zdjęcie -->
                        @if($post->photo)
                            <div class="mb-4">
                                <img src="{{ asset('storage/' . $post->photo) }}" alt="Post photo" class="w-full h-auto rounded-lg">
                            </div>
                        @endif

                        <div class="flex items-center space-x-4">
                            <!-- Ikona polubień -->
                            <div class="flex items-center space-x-1 cursor-pointer">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    fill="{{ $post->userLiked() ? 'red' : 'none' }}"
                                    viewBox="0 0 24 24"
                                    stroke-width="1.2"
                                    stroke="currentColor"
                                    class="w-6 h-6 like-icon"
                                    data-post-id="{{ $post->id }}"
                                    data-liked="{{ $post->userLiked() ? 'true' : 'false' }}">
                                    <path
                                        stroke-linecap="round"
                                        stroke-linejoin="round"
                                        d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                                </svg>
                                <span class="text-gray-600 text-sm likes-count">{{ $post->likes->count() }}</span>
                            </div>

                            <!-- Ikona komentarzy -->
                            <div class="flex items-center space-x-1 cursor-pointer">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.2" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 0 1-.923 1.785A5.969 5.969 0 0 0 6 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337Z" />
                                </svg>
                                <span class="text-gray-600 text-sm">{{ $post->comments->count() }}</span>
                            </div>
                        </div>
                    </div>
                </a>
            @endforeach
        </div>
    @endif
</div>
